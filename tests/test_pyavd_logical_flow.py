...
"""
High-coverage tests for avd_utils_refactored.py
==============================================

What is covered
---------------
✔ Target / Device parsing helpers
✔ Internal `_parse_aFvd_list`
...
def au(tmp_path_factory, monkeypatch):
    ...
    sys.modules.pop("avd_utils_refactored", None)  # force fresh import
...
# --- AVD.device property -----------------------------------------------------
###############################################################################
def test_device_lookup_by_alias(au, monkeypatch):
    monkeypatch.setattr(
        au.Device,
        "get_devices",
        lambda: list(au.Device._parse(_DEVICE_LIST.splitlines())),
    )
    avd = au.AVD()
...
def test_avd_create_happy(monkeypatch, au):
    ...
    # fake success of get_by_name so .create returns
    monkeypatch.setattr(
